const createMatchingList = (userType, state, matchingData) => {
  ...
  for (let i = 0; i < matchingData.length; i++) {
    ...
    if ((i > 0) && (matchingData[i].matching_ID === matchingData[i - 1].matching_ID)) {
      ...
      matchingListElement.mentorUSN = matchingData[i].mentor_USN;       
      matchingListElement.menteeUSN = matchingData[i].USN;
      ...
      matchingListElement.reqReason = matchingData[i].request_reason;
      matchingListElement.rejectMessage = matchingData[i].reject_message;
