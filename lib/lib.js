exports.createMatchingList = (userType, state, matchingData) => {
  let result = {};
  let matchingList = [];
  for (let i = 0; i < matchingData.length; i++) {
    ...
    if ((i > 0) && (matchingData[i].matching_ID === matchingData[i - 1].matching_ID)) {
      ...
      userType ? matchingListElement.mentee_name = matchingData[i].userName : matchingListElement.mentor_name = matchingData[i].userName;
      matchingListElement.time_req = matchingData[i].matching_request_time;
      matchingListElement.time_res = matchingData[i].matching_response_time;
      matchingListElement.state = matchingData[i].matching_state;
      matchingListElement.req_reason = matchingData[i].request_reason;
      matchingListElement.reject_message = matchingData[i].reject_message;
